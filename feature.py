...
class FeatureAgent(MahjongGBAgent):
    ...
    def request2obs(self, request):
        ...
        if t[0] == 'Draw':
            ...
            self.tileWall[0] -= 1
            self._remaining_tiles_embedding_update()
    ...
    def _packs_embedding_update(self):
        self.obs[self.OFFSET_OBS['ACTION']:self.OFFSET_OBS['ACTION'] + 25] = 0
        for player in range(4):
            offset = self.OFFSET_OBS['ACTION'] + player * 6
            for pack in self.packs[player]:
                pack_type, tile, _ = pack
                if tile != 'CONCEALED':
                    if pack_type == 'CHI':
                        self.obs[offset:offset + 4, self.OFFSET_TILE[tile]] = 1
                    elif pack_type == 'PENG':
                        self.obs[offset + 4, self.OFFSET_TILE[tile]] = 1
                    elif pack_type == 'GANG':
                        self.obs[offset + 5, self.OFFSET_TILE[tile]] = 1
            if player == 0:
                for pack in self.packs[player]:
                    pack_type, tile, _ = pack
                    if pack_type == 'GANG':
    ...
    def _history_embedding_update(self):
        self.obs[self.OFFSET_OBS['HISTORY']:self.OFFSET_OBS['HISTORY'] + 112] = 0
        for player in range(4):
            offset = self.OFFSET_OBS['HISTORY'] + player * 28
            for i, tile in enumerate(self.history[player]):
                self.obs[offset + i, self.OFFSET_TILE[tile]] = 1

    def _remaining_tiles_embedding_update(self):
        self.obs[self.OFFSET_OBS['TILES']:self.OFFSET_OBS['TILES'] + 4, :] = 0
        for i in range(4):
            self.obs[self.OFFSET_OBS['TILES'] + i, :] = self.tileWall[i]
